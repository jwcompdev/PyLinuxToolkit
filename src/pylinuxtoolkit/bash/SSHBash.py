        ...
        try:
            ...
            if print_ssh_login_success:
                self._output_writer.write(
                    f"SSH Login with {self._ssh_username} succeeded!"
                )
            ...
            self._ssh_internal_hostname = self._internal_run_ssh_command_string(
                "hostname", self._ssh_client
            )
            self._ssh_cwd = self._internal_run_ssh_command_string(
                "pwd", self._ssh_client
            )
            self._ssh_home = self._internal_run_ssh_command_string(
                "echo ~", self._ssh_client
            )
        ...
        try:
            if not self._ssh_client.closed:
                ...
                with pxssh.pxssh(encoding="utf-8", timeout=timeout) as client:
                    ...
                    if print_ssh_connection_msgs:
                        self._output_writer.write(
                            f"SSH Connecting to {self._ssh_hostname}!"
                        )
                    ...
                    exit_code = self._internal_run_ssh_command_string(
                        "echo $?", self._ssh_client
                    )
